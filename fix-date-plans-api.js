...
// ── Step 1: Create /api/date-plans route ─────────────────────────────────────
const apiDir = path.join('app', 'api', 'date-plans')
...
`)
console.log('✓ Created /api/date-plans/route.ts')

// ── Step 2: Check if dbUpsert exists in supabase lib ─────────────────────────
const supabasePath = path.join('lib', 'supabase.ts')
let supabase = fs.readFileSync(supabasePath, 'utf8')

if (!supabase.includes('dbUpsert')) {
  const OLD_EXPORT = `export { sb, dbSelect, dbInsert, dbUpdate, dbDelete }`
  const NEW_EXPORT = `export async function dbUpsert(table: string, row: Record<string, any>, onConflict: string) {
  return sb(\`/\${table}?on_conflict=\${onConflict}\`, {
    method: 'POST',
...
export { sb, dbSelect, dbInsert, dbUpdate, dbDelete, dbUpsert }`

  if (supabase.includes(OLD_EXPORT)) {
    supabase = supabase.replace(OLD_EXPORT, NEW_EXPORT)
    fs.writeFileSync(supabasePath, supabase, 'utf8')
    console.log('✓ Added dbUpsert to lib/supabase.ts')
  } else {
    // Add dbUpsert before last export
    supabase += `
export async function dbUpsert(table: string, row: Record<string, any>, onConflict: string) {
  return sb(\`/\${table}?on_conflict=\${onConflict}\`, {
    method: 'POST',
    body: JSON.stringify(row),
    headers: { 'Prefer': 'resolution=merge-duplicates,return=representation' },
    ...
    fs.writeFileSync(supabasePath, supabase, 'utf8')
    console.log('✓ Added dbUpsert to lib/supabase.ts (appended)')
  }
} else {
  console.log('✓ dbUpsert already exists in lib/supabase.ts')
}
