...
// 1. Add ReprocessModal import
if (!faulty.includes('ReprocessModal')) {
  faulty = `import ReprocessModal from '@/components/ReprocessModal'\n` + faulty
  console.log('✓ Added ReprocessModal import')
}

// 2. Update handleReprocess to accept data parameter (full/partial)
...
console.log('✓ Updated handleReprocess for full/partial')

// 3. Replace old reprocess modal with ReprocessModal component
const oldModal = `      {/* Reprocess Modal */}
      {reprocessModal && (
...
const newModal = `      {/* Reprocess Modal — Full or Partial */}
      {reprocessModal && (
        <ReprocessModal
...
if (faulty.includes(oldModal)) {
  faulty = faulty.replace(oldModal, newModal)
  console.log('✓ Replaced reprocess modal with ReprocessModal component')
} else {
  console.error('✗ Old reprocess modal not found in faulty page')
}
...
if (!fob.includes('ReprocessModal')) {
  ...
  console.log('✓ Added ReprocessModal import to FOB')
}

// 2. Add reprocessModal state
fob = fob.replace(
...
// 3. Add handlers before handleUpdate
fob = fob.replace(
...
console.log('✓ Added FOB handlers')
...
console.log('✓ Updated FOB actions column')

// 5. Add ReprocessModal before closing
fob = fob.replace(
  `    </div>
  )
}
