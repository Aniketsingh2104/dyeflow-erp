    ...
    <div className="content">
      <div className="card" style={{maxWidth:620,margin:"40px auto"}}>
        <div className="card-header">
          <span className="card-title">ðŸ”„ Migrate Data to Server</span>
        </div>
        ...
        {message && (
          <div style={{padding:"12px 16px",borderRadius:8,marginBottom:16,fontSize:13,
            ...
            {status==="running"&&"â³ "}{status==="done"&&"âœ… "}{status==="error"&&"âŒ "}
            {message}
          </div>
        )}
        {stats && status==="done" && (
        ...
        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
          <button onClick={migrate} disabled={status==="running"} className="primary"
            ...
            {status==="running"?"â³ Migrating...":"ðŸš€ Migrate to Server"}
          </button>
          <button onClick={verify} style={{padding:"10px 20px",fontSize:13}}>
            ðŸ” Verify Server Data
          </button>
        </div>
        {status==="done" && (
          <div style={{marginTop:20,padding:"14px 16px",background:"var(--bg-secondary)",
            ...
            â€¢ All devices now share the same data<br/>
            â€¢ Changes sync automatically every 5 seconds<br/>
            â€¢ Data stored in: <code>data/dyeflow_db.json</code>
          </div>
        )}
      </div>
    </div>
