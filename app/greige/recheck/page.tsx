...
interface GreigeRecheckOrder {
  ...
  remarks: string
  supervisor: string
  greigeCheckAt: string
  plannedDate: string
  greigeRecheckStatus?: 'Pass' | 'Fail' | ''
  greigeRecheckDone?: boolean
  greigeRecheckDoneAt?: string
  greigeRecheckFailReason?: string
}
...
export default function GreigeRecheckPage() {
  ...
  const [currentFailOrderId, setCurrentFailOrderId] = useState('')
  ...
  const loadColumnConfig = () => {
    const saved = localStorage.getItem('greige_recheck_columns')
    if (saved) {
      try {
  ...
  const saveColumnConfig = () => {
    localStorage.setItem('greige_recheck_columns', JSON.stringify(columns))
  }

  const resetColumns = () => {
    setColumns(DEFAULT_COLUMNS)
    localStorage.removeItem('greige_recheck_columns')
  }
  ...
  const loadData = () => {
    ...
    const recheckOrders = (db.orders || [])
      .filter((o: any) => o.greigeCheck === true)
      .map((o: any) => {
        const greigeCheckAt = o.greigeCheckAt || new Date().toISOString()
        const plannedDate = new Date(greigeCheckAt)
        ...
        return {
          ...
          greigeRecheckStatus: o.greigeRecheckStatus || '',
          greigeRecheckDone: o.greigeRecheckDone || false,
          greigeRecheckDoneAt: o.greigeRecheckDoneAt || '',
          greigeRecheckFailReason: o.greigeRecheckFailReason || ''
        }
      })
      .sort((a: any, b: any) => 
        new Date(b.greigeCheckAt).getTime() - new Date(a.greigeCheckAt).getTime()
      )

    setOrders(recheckOrders)
  }

  const setGreigeRecheckStatus = (orderId: string, status: 'Pass' | 'Fail' | '') => {
    const stored = localStorage.getItem('dyeflow_db')
    if (!stored) return
    ...
    if (nextStatus === 'Fail') {
      // Open custom modal for fail reason
      setCurrentFailOrderId(orderId)
      setFailReasonInput(order.greigeRecheckFailReason || '')
      setShowFailReasonModal(true)
      return
    } else if (nextStatus === 'Pass') {
      order.greigeRecheckFailReason = ''
    } else {
      order.greigeRecheckFailReason = ''
    }

    order.greigeRecheckStatus = nextStatus
    order.greigeRecheckDone = nextStatus === 'Pass' || nextStatus === 'Fail'
    order.greigeRecheckDoneAt = order.greigeRecheckDone ? new Date().toISOString() : ''

    localStorage.setItem('dyeflow_db', JSON.stringify(db))
    loadData()
  }

  const handleFailReasonSubmit = () => {
    if (!failReasonInput.trim()) {
      alert('Fail reason is required.')
      return
    }
    ...
    const db = JSON.parse(stored)
    const order = db.orders.find((o: any) => o.id === currentFailOrderId)
    if (!order) return

    order.greigeRecheckFailReason = failReasonInput.trim()
    order.greigeRecheckStatus = 'Fail'
    order.greigeRecheckDone = true
    order.greigeRecheckDoneAt = new Date().toISOString()
  ...
  const getDelayInfo = (order: GreigeRecheckOrder) => {
    const planned = new Date(order.plannedDate)
    const ref = order.greigeRecheckDoneAt 
      ? new Date(order.greigeRecheckDoneAt) 
      : new Date()
    ...
    if (order.greigeRecheckDoneAt && diff >= 0) {
  ...
  const renderCell = (order: GreigeRecheckOrder, col: ColumnConfig) => {
    const baseStyle = {
      ...cellStyle,
      width: `${col.width}px`,
    ...
    switch (col.key) {
      case 'timestamp':
        return <td key={col.key} style={{ ...baseStyle, fontSize: '11px', whiteSpace: 'nowrap' }}>{formatDateTime(order.greigeCheckAt)}</td>
      case 'orderNumber':
      ...
      case 'remarks':
        return (
          <td key={col.key} style={{ ...baseStyle, maxWidth: '220px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} title={order.remarks}>
            {order.remarks || '-'}
          </td>
        )
      case 'plannedDate':
      ...
      case 'actualDate':
        return <td key={col.key} style={{ ...baseStyle, fontSize: '11px', whiteSpace: 'nowrap' }}>{order.greigeRecheckDoneAt ? formatDateTime(order.greigeRecheckDoneAt) : '-'}</td>
      case 'status':
        return (
          <td key={col.key} style={{ ...baseStyle, textAlign: 'center' }}>
            <select
              value={order.greigeRecheckStatus || ''}
              onChange={(e) => setGreigeRecheckStatus(order.id, e.target.value as any)}
              style={{
                fontSize: '12px',
                padding: '4px 8px',
                minWidth: '90px',
  ...
  const done = orders.filter(o => o.greigeRecheckDone).length
  ...
  return (
    <div className="content" style={{ padding: '20px', maxWidth: '100%', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          <div>
            ...
            <p style={{ fontSize: '13px', opacity: 0.9, margin: 0 }}>
              Quality verification for greige fabric orders marked by supervisors
            </p>
          </div>
          <button
      ...
      <div style={{
        ...
        {[
          { label: 'Greige Recheck', value: orders.length, sub: 'Marked by supervisors', color: '#1a1a18' },
          ...
          { label: 'Supervisors', value: supervisors.length, sub: 'With recheck orders', color: '#10B981' }
        ].map((stat, idx) => (
          <div key={idx} style={{
            minWidth: '165px',
            flex: '0 0 165px',
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <h3 style={{ fontSize: '15px', fontWeight: 600, margin: 0 }}>Greige Recheck Register</h3>
        ...
        {orders.length === 0 ? (
          <div style={{
            ...
            <div style={{ fontSize: '15px', fontWeight: 600, color: '#5a5a55', marginBottom: '6px' }}>
              No greige recheck orders yet
            </div>
            <div style={{ fontSize: '13px' }}>
              Orders will appear here when supervisors mark them for greige recheck
            </div>
          </div>
        ) : (
                    ...
                    >
                      ...
                      <div
                        ...
                        onMouseEnter={(e) => {
                          ...
                          indicator.style.cssText = 'position:absolute;top:20%;bottom:20%;right:3px;width:2px;background:#10B981;border-radius:2px;pointer-events:none'
      ...
      {showFailReasonModal && (
        ...
        >
          ...
          >
            <h3 style={{ fontSize: '18px', fontWeight: 600, margin: 0, marginBottom: '16px' }}>
              Enter Greige Recheck fail reason:
            </h3>
            ...
            <input
              ...
              placeholder="Type fail reason here..."
              autoFocus
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  handleFailReasonSubmit()
                }
              }}
              style={{
                width: '100%',
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              ...
              <button
                onClick={handleFailReasonSubmit}
                style={{
                  padding: '10px 20px',
                  fontSize: '14px',
                  border: 'none',
              ...
              {columns.map(col => (
                <label
                  ...
                  style={{
                    ...
                    borderRadius: '6px',
                    cursor: 'pointer',
                    background: col.visible ? '#f0fdf4' : 'white',
