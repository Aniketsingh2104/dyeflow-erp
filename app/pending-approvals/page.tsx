...
export default function PendingApprovalsPage() {
  ...
  const handleApprove = (item: PendingItem) => {
    ...
    // Update row with approval and receivedAt timestamp
    const now = new Date().toISOString()
    row.approvalStatus = 'approved'
    row.orderNumber = newOrder.orderNumber
    row.submitForApproval = false
    row.requestEdit = false
    row.receivedAt = now  // Set timestamp when approved
  ...
  const handleReject = () => {
    ...
    // Update row with rejection and receivedAt timestamp
    const now = new Date().toISOString()
    row.approvalStatus = 'rejected'
    row.submitForApproval = false
    row.requestEdit = false
    row.rejectionReason = rejectReason
    row.receivedAt = now  // Set timestamp when rejected
      ...
      <div className="card">
        ...
        {pendingItems.length === 0 ? (
          ...
          <div className="table-wrap" style={{ overflowX: 'auto' }}>
            <table style={{ minWidth: '2200px' }} className="sticky-table">
              <thead>
                <tr>
                  <th>Sheet</th>
                  <th>Row</th>
