...
interface FactorySettings {
  orderPrefix: string        // e.g. DYG, FAB, TXT
  factoryName: string
  city: string
  gstNumber: string
  phone: string
...
function loadSettings(): FactorySettings {
  try {
    ...
    return { ...DEFAULTS, ...(db.settings?.factory || {}) }
  } catch { return { ...DEFAULTS } }
}

function saveSettings(s: FactorySettings) {
  const raw = localStorage.getItem('dyeflow_db')
  const db = raw ? JSON.parse(raw) : {}
  if (!db.settings) db.settings = {}
  db.settings.factory = s
  ...
  const Field = ({ label, field, placeholder, note }: { label: string; field: keyof FactorySettings; placeholder?: string; note?: string }) => (
    <div style={{ marginBottom: 18 }}>
      ...
      <input
        ...
        style={{ width: '100%', maxWidth: 340, padding: '8px 12px', fontSize: 14, border: '1px solid var(--border-medium)', borderRadius: 6 }}
      />
  ...
  return (
    <div className="content" style={{ maxWidth: 700, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--text-primary)' }}>⚙ Factory Settings</div>
      ...
      {saved && (
        <div style={{ background: '#D1FAE5', color: '#065F46', border: '1px solid #6EE7B7', borderRadius: 8, padding: '10px 16px', marginBottom: 16, fontSize: 13, fontWeight: 600 }}>
          ✓ Settings saved successfully
        </div>
      )}

      {/* Order Number Prefix */}
      <div style={{ background: 'var(--bg-primary)', border: '1px solid var(--border-light)', borderRadius: 12, padding: '20px 24px', marginBottom: 16 }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 4 }}>Order Number Format</div>
        ...
        <Field
          ...
          field="orderPrefix"
          placeholder="e.g. DYG, FAB, TXT"
          note="3–6 uppercase letters. Will be auto-uppercased on save."
        />

        {/* Live preview */}
        <div style={{ background: 'var(--bg-secondary)', borderRadius: 8, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <button
            onClick={() => updatePreview(form.orderPrefix)}
        ...
        <div style={{ marginTop: 12, padding: '10px 14px', background: '#FEF3C7', borderRadius: 8, fontSize: 12, color: '#92400E' }}>
          ⚠️ Changing the prefix only affects <strong>new orders</strong> created after saving. Existing order numbers are not renamed.
        </div>
      </div>

      {/* Factory Info */}
      <div style={{ background: 'var(--bg-primary)', border: '1px solid var(--border-light)', borderRadius: 12, padding: '20px 24px', marginBottom: 16 }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 16 }}>Factory Information</div>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <button
          onClick={handleSave}
          style={{ padding: '10px 24px', fontSize: 14, fontWeight: 700, border: 'none', borderRadius: 8, background: 'var(--accent)', color: '#fff', cursor: 'pointer' }}
        >
          Save Settings
        </button>
